...
@section('content')
    ...
    <main class="w-full mt-16" >
        <header class="py-3  w-full md:h-24  bg-gray-100 ">
            <div class="container lg:px-32 flex flex-col md:flex-row md:justify-between items-center ">
            ...
            <div class="mt-4 mt-md-0">
                <form action="/search" method="get"  class=" w-full h-12 md:w-2/5 md:h-4/6 bg-white flex flex-row rounded ">
                    ...
                    <input id="search" type="search"  name="search"
                        ...
                        required
                    >
                    <button type="submit" class="bg-green rounded-r-sm text-white"><i class="w-8 fas fa-search"></i></button>
                </form>
            </div>
        </header>

        {{-- Product --}}
        <div class="container lg:px-32 w-full flex flex-col pt-4 ">
        ...
        <section class="container lg:px-32 py-4 w-full flex flex-col lg:flex-row ">
            <!-- gallery -->
            <div class="w-full mt-3">
                <div class="grid gap-5 grid-cols-1 lg:grid-cols-2">
                    @forelse ($products as $product)
                        <div class="cursor-pointer box w-full d-flex flex-column flex-md-row mb-4 " >
                            <div class="h-full md:w-1/3 d-flex justify-content-center rounded-l-md bg-white">
                            ...
                            <div class="flex-1 relative flex flex-col justify-between bg-gray-50 rounded-r-md px-4 pt-1 pb-3">
                                <a href="/producto/{{$product->slug}}" style="text-decoration: none;">
                                ...
                                {{-- @if($cartid && $product && $product->id && $product->qty) --}}
                                <cart-add :cartid={{ $cartid }} :id={{ $product->id }} :stock={{ $product->qty ?? 0 }} ></cart-add>
                                {{-- @endif --}}
                            </div>
                        </div>
                    @empty
