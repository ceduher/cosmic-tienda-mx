<form  class="mt-8" method="POST" action="/login">
    ...
    {{-- Forgot Your Password --}}
        ...
        @if (Route::has('password.request'))
            <div class="my-3">
                ...
                <a class="text-green text-sm no-underline hover:underline hover:text-gray-400">
                    {{__('Forgot Your Password?') }}  <br> Le ayudamos, por favor contáctenos al 999 114 5236
                </a>
            </div>
        @endif
    ...
    {{-- Submit Button --}}
        <div class="flex flex-row-reverse">
            <button type="submit" class="btn bg-green mb-1 px-5 py-2.5 text-white btn-block">
                {{__('Submit')}}
            </button>
        </div>
