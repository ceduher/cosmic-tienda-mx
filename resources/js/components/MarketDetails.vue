<template>
  <div class="relative w-full container flex flex-col lg:flex-row pb-5">  
    ...
    <div class="w-full rounded lg:w-1/3">
      ...
      <div class="w-full border-2 rounded border-gray-300 border-dotted">
        ...
        <div v-if="market" class="py-3 px-4 flex flex-col">
          ...
          <br/>
          <div @change="checkPrice(ordersTotalPrice)" class="flex flex-row justify-between my-2">
      ...
      <div v-show="desc" class="my-3 flex flex-row justify-around p-6 bg-green-200">
        <span class="text-center font-semibold">Te faltan $ {{ (400 -  ordersTotalPrice).toFixed(2) }} para tener envio gratis</span>
      </div>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
        ...
        showPrice(price = 0) {
           if (price >= 400 && this.applyCoupon == false){

            this.ordersTotalPrice = price - 35;
            this.desc = false;
            this.market.delivery_fee = 0;
        ...
        addToOrders(product) {
          ...
          let relative_price = orderPrice * this.numberOfMeals;
          if (relative_price >= 400){

            this.ordersTotalPrice = (this.ordersTotalPrice + relative_price) - 35;
            this.desc = false;
        ...
        checkPrice(ordersTotalPrice) {
          if (ordersTotalPrice >= 400){

            this.ordersTotalPrice = ordersTotalPrice - 35;
            this.desc = false;
